...
import { LayoutDashboard, FolderOpen, CreditCard, BarChart2, LogOut, Menu, X, Bot, FileText, Users, Settings, User } from 'lucide-vue-next'
...
// All color tokens in one place — swap dark/light here
const vars = computed(() => ({
    '--brand':      theme.value.brand,
    '--sb-bg':      '#0f172a',           // sidebar always dark
    '--sb-border':  '#1e293b',
    '--sb-text':    '#94a3b8',
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="flex items-center gap-3 px-5 py-5 shrink-0"
                ...
                <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 overflow-hidden"
                    ...
                    <img v-if="tenant?.logo_path" :src="`/storage/${tenant.logo_path}`" class="w-full h-full object-contain p-1"/>
            ...
            <nav class="px-3 pt-4 pb-2 space-y-0.5 shrink-0">
                <template v-for="item in [
                    ...
                    <Link :href="item.href"
                          ...
                          :style="isActive(item.path) ? `background:var(--brand);color:#fff` : `color:var(--sb-text)`"
                          @mouseenter="e => { if(!isActive(item.path)) e.currentTarget.style.background='rgba(255,255,255,.07)'; e.currentTarget.style.color='#fff' }"
                          @mouseleave="e => { if(!isActive(item.path)) { e.currentTarget.style.background=''; e.currentTarget.style.color='' } }"
                          @click="sidebarOpen=false">
                        <component :is="item.icon" class="w-4 h-4 shrink-0"/> {{ item.label }}
                    </Link>
                </template>
                <Link v-if="isAdmin" :href="route('team.index')"
                      class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
                      :style="isActive('/team') ? `background:var(--brand);color:#fff` : `color:var(--sb-text)`"
                      @click="sidebarOpen=false">
                    <Users class="w-4 h-4 shrink-0"/> Team
                </Link>
                <Link :href="route('settings.index')"
                      class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
                      :style="isActive('/settings') ? `background:var(--brand);color:#fff` : `color:var(--sb-text)`"
                      @click="sidebarOpen=false">
            ...
            <!-- Projects tree -->
            <div class="flex-1 overflow-y-auto py-2" style="border-top:1px solid var(--sb-border)">
                <Sidebar :projects="sidebarProjects"/>
            </div>

            <!-- Bottom nav -->
            <nav class="px-3 py-2 space-y-0.5 shrink-0" style="border-top:1px solid var(--sb-border)">
                <a href="/billing" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
                   :style="isActive('/billing') ? `background:var(--brand);color:#fff` : `color:var(--sb-text)`">
                    <CreditCard class="w-4 h-4 shrink-0"/> Billing
                </a>
                <a href="/usage" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
                   :style="isActive('/usage') ? `background:var(--brand);color:#fff` : `color:var(--sb-text)`">
                    <BarChart2 class="w-4 h-4 shrink-0"/> Usage
                </a>
            </nav>

            <!-- User/logout -->
            <div class="px-3 py-4 shrink-0" style="border-top:1px solid var(--sb-border)">
                <div class="flex items-center gap-3 px-3 py-2 mb-1">
                    <div class="w-7 h-7 rounded-full flex items-center justify-center shrink-0" style="background:var(--sb-border)">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="flex items-center justify-between px-4 py-3 shrink-0"
                ...
                <div class="flex items-center gap-3">
                    <button class="md:hidden" style="color:var(--muted)" @click="sidebarOpen=!sidebarOpen">
            ...
            <div v-if="quota?.exceeded" class="px-4 py-2 text-xs text-center text-amber-400 bg-amber-500/8 border-b border-amber-500/15">
                Token quota exceeded. <button @click="showUpgrade=true" class="underline ml-1">Upgrade plan</button>
            </div>

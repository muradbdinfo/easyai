...
import {
  Users, UserPlus, Mail, Shield, Crown, Trash2, Edit2,
  Copy, CheckCircle, Clock, AlertCircle, X, RefreshCw,
  ChevronDown, Eye, EyeOff, MoreVertical, Link,
  UserCheck, UserX, Key
...
// ─── Flash invite URL from server ────────────────────────────────────
import { usePage } from '@inertiajs/vue3'
const page = usePage()
const serverFlash = computed(() => page.props.flash ?? {})

// Watch for invite_url in flash
import { watch } from 'vue'
watch(() => page.props.flash, (flash) => {
  if (flash?.invite_url) {
    flashUrl.value = flash.invite_url
...
function submitInvite() {
  inviteForm.post(route('team.invite'), {
    preserveScroll: true,
    onSuccess: () => {
      inviteForm.reset()
    },
...
function openEditRole(member) {
  editRoleUser.value = member
  roleForm.role = member.role
}

function saveRole() {
  roleForm.put(route('team.member.role', editRoleUser.value.id), {
    preserveScroll: true,
    onSuccess: () => { editRoleUser.value = null },
  })
}
...
function resendInvite(inv) {
  router.post(route('team.invitation.resend', inv.id), {}, {
    preserveScroll: true,
    onSuccess: () => {
      if (page.props.flash?.invite_url) {
        flashUrl.value = page.props.flash.invite_url
      }
    },
  })
}
...
// ─── Helpers ──────────────────────────────────────────────────────────
const roleBadge = {
  admin:  'bg-indigo-900/50 text-indigo-300 border border-indigo-700',
...
<template>
  <AppLayout title="Team">
    <div class="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div
        v-if="flashUrl"
        class="bg-green-900/30 border border-green-700/50 rounded-xl p-4 space-y-3"
      >
        ...
        <div class="flex items-center gap-2">
          <div class="flex-1 bg-slate-800 rounded-lg px-3 py-2 text-sm text-slate-300 font-mono truncate border border-slate-600">
            {{ flashUrl }}
          </div>
          <button
            @click="copyUrl(flashUrl)"
            class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm transition-colors"
            :class="copiedId === 'flash'
              ? 'bg-green-700 text-white'
              : 'bg-slate-700 hover:bg-slate-600 text-slate-300'"
          >
            <CheckCircle v-if="copiedId === 'flash'" class="w-4 h-4" />
            <Copy v-else class="w-4 h-4" />
            {{ copiedId === 'flash' ? 'Copied!' : 'Copy' }}
          </button>
          <button @click="flashUrl = null" class="p-2 text-slate-500 hover:text-slate-300">
            <X class="w-4 h-4" />
          </button>
        </div>
      ...
      <div class="flex gap-1 bg-slate-800/50 rounded-xl p-1 w-fit">
        <button
          v-for="tab in [
            ...
            { key: 'invitations', label: 'Pending',     count: invitations.length },
          ]"
          :key="tab.key"
        ...
        >
          ...
          <span
            ...
            :class="activeTab === tab.key ? 'bg-slate-600 text-white' : 'bg-slate-700 text-slate-400'"
          >
            {{ tab.count }}
          </span>
        </button>
      ...
      <div v-if="activeTab === 'members'" class="bg-slate-800/40 rounded-xl border border-slate-700/50 overflow-hidden">
        ...
        <div v-else class="divide-y divide-slate-700/50">
          <div
            ...
            :key="member.id"
            class="flex items-center gap-4 px-5 py-4 hover:bg-slate-700/20 transition-colors"
          >
            <!-- Avatar -->
            <div
              class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0"
              :class="member.role === 'admin' ? 'bg-indigo-600 text-white' : 'bg-slate-600 text-slate-200'"
            >
            ...
            <span
              ...
              :class="roleBadge[member.role] ?? 'bg-slate-700 text-slate-300'"
            >
            ...
            <!-- Status badge -->
            <span
              class="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full flex-shrink-0"
            ...
            <!-- Joined -->
            <span class="text-slate-500 text-xs flex-shrink-0 hidden md:block">
              {{ member.joined_at }}
            </span>
            ...
            <div v-if="canManage && !member.is_self" class="flex items-center gap-1 flex-shrink-0">
              ...
              <button
                ...
                class="p-1.5 text-slate-400 hover:text-white hover:bg-slate-600 rounded-lg transition-colors"
                title="Change role"
              >
              ...
              >
                <UserX v-if="member.is_active" class="w-3.5 h-3.5" />
                <UserCheck v-else class="w-3.5 h-3.5" />
              </button>
              <!-- Remove -->
              <button
                @click="removeMember(member)"
                class="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/30 rounded-lg transition-colors"
      ...
      <div v-if="activeTab === 'invitations'" class="bg-slate-800/40 rounded-xl border border-slate-700/50 overflow-hidden">
        ...
        <div v-else class="divide-y divide-slate-700/50">
          ...
          >
            <!-- Icon -->
            <div class="w-9 h-9 rounded-full bg-yellow-900/30 flex items-center justify-center flex-shrink-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-slate-400 text-xs">
                Invited {{ inv.created_at }} by {{ inv.inviter }}
            ...
            <span
              class="px-2 py-0.5 text-xs rounded-full font-medium flex-shrink-0"
              :class="roleBadge[inv.role]"
            >
              {{ roleLabel[inv.role] }}
            </span>

            <!-- Status -->
            <span class="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-yellow-900/40 text-yellow-400 flex-shrink-0">
              <Clock class="w-3 h-3" />
              Pending
            </span>
            ...
            <div v-if="canManage" class="flex items-center gap-1 flex-shrink-0">
              <!-- Copy link -->
              <button
                @click="copyUrl(inv.invite_url, inv.id)"
                class="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs transition-colors"
                :class="copiedId === inv.id
                  ? 'bg-green-700 text-white'
                  : 'bg-slate-700 hover:bg-slate-600 text-slate-300'"
                title="Copy invite link"
              >
                <CheckCircle v-if="copiedId === inv.id" class="w-3.5 h-3.5" />
        ...
        <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl">
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700">
            <div class="flex items-center gap-2">
              ...
              <h2 class="text-white font-semibold">Invite Team Member</h2>
            </div>
            <button @click="showInvite = false" class="text-slate-400 hover:text-white">
              <X class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full bg-slate-800 border border-slate-600 rounded-lg pl-10 pr-4 py-2.5
                         text-white placeholder-slate-500 text-sm focus:outline-none
                         focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                  @keyup.enter="submitInvite"
                />
              </div>
              <p v-if="inviteForm.errors.email" class="text-red-400 text-xs mt-1">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button
                  v-for="opt in [
                    { value: 'member', label: 'Member',  desc: 'Can use chats & projects',   icon: Users },
                    { value: 'admin',  label: 'Admin',   desc: 'Can manage team & settings', icon: Shield },
                  ]"
                  :key="opt.value"
                  @click="inviteForm.role = opt.value"
                  class="flex flex-col items-start p-3 rounded-xl border text-left transition-colors"
                  :class="inviteForm.role === opt.value
                    ? 'border-indigo-500 bg-indigo-600/20'
                    : 'border-slate-600 bg-slate-800 hover:border-slate-500'"
                >
                  <div class="flex items-center gap-2 mb-1">
                    <component :is="opt.icon" class="w-3.5 h-3.5"
                      :class="inviteForm.role === opt.value ? 'text-indigo-400' : 'text-slate-400'" />
                    <span class="text-sm font-medium"
                      :class="inviteForm.role === opt.value ? 'text-white' : 'text-slate-300'">
                      {{ opt.label }}
                    </span>
                  </div>
    ...
    <Teleport to="body">
      <div
        v-if="editRoleUser"
        class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        @click.self="editRoleUser = null"
      >
        <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm shadow-2xl">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700">
            <div class="flex items-center gap-2">
              <Key class="w-5 h-5 text-indigo-400" />
              <h2 class="text-white font-semibold">Change Role</h2>
            </div>
            <button @click="editRoleUser = null" class="text-slate-400 hover:text-white">
              <X class="w-5 h-5" />
            </button>
          </div>
          <div class="px-6 py-5 space-y-4">
            <p class="text-slate-400 text-sm">
              Change role for <span class="text-white font-medium">{{ editRoleUser.name }}</span>
            </p>
            <div class="grid grid-cols-2 gap-2">
              <button
                v-for="r in ['admin', 'member']"
                :key="r"
                @click="roleForm.role = r"
                class="p-3 rounded-xl border text-sm font-medium transition-colors capitalize"
                :class="roleForm.role === r
          ...
          <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-700">
            <button @click="editRoleUser = null" class="px-4 py-2 text-slate-400 hover:text-white text-sm">
              Cancel
            </button>
            <button
              @click="saveRole"

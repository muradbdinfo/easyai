...
import {
    User, Mail, Lock, Settings, Building2, Image,
    Key, Bell, Save, Eye, EyeOff, Plus, Trash2,
    Copy, CheckCircle, AlertCircle, Zap
...
const workspaceForm = useForm({
    ...
    default_model: props.tenant.default_model,
})
...
function createKey() {
    ...
    keyForm.post(route('settings.apikey.create'), {
        preserveScroll: true,
        onSuccess: () => { newKeyName.value = '' },
    })
}
    ...
    <AppLayout title="Settings">
        <div class="max-w-3xl mx-auto px-4 py-8">
            ...
            <div v-if="flash.success"
                 class="mb-4 flex items-center gap-2 bg-green-900/30 border border-green-700/40
                        text-green-400 text-sm px-4 py-2.5 rounded-lg">
            ...
            <!-- New token banner -->
            <div v-if="newToken"
                 class="mb-4 bg-amber-900/30 border border-amber-700/40 rounded-lg p-4">
                <p class="text-amber-400 text-xs font-semibold mb-1">
                    Copy your new API key — it won't be shown again.
                </p>
                <div class="flex items-center gap-2">
                    <code class="flex-1 bg-slate-900 text-green-400 text-xs px-3 py-2 rounded font-mono break-all">
                        {{ newToken }}
                    </code>
                    <button @click="copyToken"
                            class="shrink-0 p-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors">
                        <CheckCircle v-if="copiedToken" class="w-4 h-4 text-green-400" />
            ...
            <div v-if="activeTab === 'profile'" class="space-y-5">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <form @submit.prevent="profileForm.put(route('settings.profile'), { preserveScroll: true })"
                          class="space-y-4">
                        <div>
                            ...
                            <p v-if="profileForm.errors.name" class="text-red-400 text-xs mt-1">
                        ...
                        <div>
                            ...
                            <input v-model="profileForm.email" type="email" required
                                   class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2
                                          text-white text-sm outline-none focus:border-indigo-500 transition-colors" />
                            <p v-if="profileForm.errors.email" class="text-red-400 text-xs mt-1">
                                {{ profileForm.errors.email }}
                            </p>
                        </div>
                        <div class="flex items-center gap-3 pt-1">
                            <span class="text-xs text-slate-500 capitalize">Role: {{ user.role }}</span>
                            <button type="submit" :disabled="profileForm.processing"
                                    class="ml-auto flex items-center gap-2 px-4 py-2 bg-indigo-600
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <form @submit.prevent="passwordForm.put(route('settings.password'), { preserveScroll: true })"
                          class="space-y-4">
                        <!-- Current -->
                        <div>
                            ...
                            <div class="relative">
                                <input v-model="passwordForm.current_password"
                                       ...
                                       class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2
                                              text-white text-sm outline-none focus:border-indigo-500 pr-10" />
                            ...
                            <p v-if="passwordForm.errors.current_password" class="text-red-400 text-xs mt-1">
            ...
            <div v-if="activeTab === 'workspace'" class="space-y-5">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <div class="flex items-center gap-5">
                        <!-- Preview -->
                        <div class="w-20 h-20 rounded-xl bg-slate-800 border border-slate-700
                        ...
                        <div class="flex-1 space-y-2">
                            ...
                            <div class="flex gap-2 flex-wrap">
                                <label v-if="canManage"
                                       class="cursor-pointer flex items-center gap-2 px-3 py-1.5
                                              bg-slate-700 hover:bg-slate-600 text-slate-200 text-xs
                                              rounded-lg transition-colors">
                                    <Image class="w-3.5 h-3.5" />
                                    Choose File
                                    <input type="file" accept="image/*" class="hidden" @change="onLogoSelect" />
                                </label>
                                <button v-if="logoFile && canManage"
                                        @click="uploadLogo" :disabled="logoUploading"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-indigo-600
                                               hover:bg-indigo-500 text-white text-xs rounded-lg
                                               disabled:opacity-50 transition-colors">
                                    <Save class="w-3.5 h-3.5" />
                                    {{ logoUploading ? 'Uploading...' : 'Upload' }}
                                </button>
                                <button v-if="logoPreview && canManage"
                                        @click="deleteLogo"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-red-900/40
                                               hover:bg-red-800/50 text-red-400 text-xs rounded-lg transition-colors">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <div v-if="!canManage"
                         class="flex items-center gap-2 text-amber-400 text-xs mb-4">
                        <AlertCircle class="w-4 h-4" />
                        Only admins can edit workspace settings.
                    </div>

                    <form @submit.prevent="workspaceForm.put(route('settings.workspace'), { preserveScroll: true })"
                          class="space-y-4">
                        <div>
                        ...
                        <div>
                            ...
                            <select v-model="workspaceForm.default_model"
                                    :disabled="!canManage"
                                    class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2
                                           text-white text-sm outline-none focus:border-indigo-500
                                           disabled:opacity-50 transition-colors">
                        ...
                        <div class="pt-1">
                            <div class="text-xs text-slate-500">
                                Workspace slug: <span class="text-slate-400 font-mono">{{ tenant.slug }}</span>
                            </div>
                        </div>
            ...
            <!-- ── API Keys Tab ─────────────────────────────────── -->
            <div v-if="activeTab === 'api_keys'" class="space-y-5">
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <p class="text-slate-500 text-xs mb-5">
                        Use these tokens to access the EasyAI API from external apps or mobile.
                    </p>

                    <!-- Create -->
                    <div class="flex gap-2 mb-5">
                        <input v-model="newKeyName" type="text" placeholder="Key name (e.g. Mobile App)"
                               class="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2
                                      text-white text-sm outline-none focus:border-indigo-500" />
                        <button @click="createKey"
                                class="flex items-center gap-2 px-4 py-2 bg-indigo-600
                                       hover:bg-indigo-500 text-white text-sm rounded-lg transition-colors">
                            <Plus class="w-4 h-4" /> Create
                        </button>
                    </div>

                    <!-- List -->
                    <div v-if="api_tokens.length === 0" class="text-slate-500 text-sm text-center py-6">
                        No API keys yet.
                    </div>
                    <div v-else class="space-y-2">
                        <div v-for="token in api_tokens" :key="token.id"
                            ...
                            <button @click="deleteKey(token.id)"
                                    class="text-red-400 hover:text-red-300 transition-colors p-1.5">
            ...
            <div v-if="activeTab === 'notifications'">
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <form @submit.prevent="notifForm.put(route('settings.notifications'), { preserveScroll: true })"
                          class="space-y-4">
                        ...
                        <label v-for="(label, key) in {
                            ...
                            team_invitation: 'Team invitation received',
                               ...
                               class="flex items-center justify-between bg-slate-800 rounded-lg px-4 py-3 cursor-pointer">
                            <span class="text-slate-300 text-sm">{{ label }}</span>
                            <div class="relative">
                                <input type="checkbox" v-model="notifForm[key]" class="sr-only peer" />
                                <div class="w-9 h-5 bg-slate-600 peer-checked:bg-indigo-600
                                            rounded-full transition-colors peer-focus:ring-2
                                            peer-focus:ring-indigo-500/50"></div>

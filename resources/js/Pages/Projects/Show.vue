        ...
        <div class="max-w-5xl mx-auto px-6 py-8">
            ...
            <div class="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 mb-6 w-fit">
                <button
                    ...
                    :class="activeTab === 'chats' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'"
                    class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all"
                >
                    <MessageSquare class="w-4 h-4" />
                    Chats
                    <span class="bg-slate-700 text-slate-300 text-xs px-1.5 py-0.5 rounded-full">{{ project.chats_count }}</span>
                </button>
                <button
                    @click="activeTab = 'settings'"
                    :class="activeTab === 'settings' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'"
                    class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all"
                >
                ...
                <div v-else class="space-y-2">
                    ...
                    >
                        <div class="flex items-center gap-4 min-w-0">
                            ...
                            <div class="min-w-0">
                                <p class="text-white font-medium text-sm truncate">{{ chat.title || 'New Chat' }}</p>
                                <div class="flex items-center gap-1.5 mt-0.5">
                                    <Clock class="w-3 h-3 text-slate-600" />
                                    <span class="text-xs text-slate-500">{{ formatTime(chat.updated_at) }}</span>
                                    <span v-if="chat.total_tokens" class="text-xs text-slate-600">
                                        &middot; {{ chat.total_tokens }} tokens
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div v-if="activeTab === 'settings'">
                <form @submit.prevent="saveSettings" class="space-y-6 max-w-2xl">
                    ...
                    <div>
                        ...
                        />
                        <p class="text-slate-500 text-xs mt-1">Sets the AI personality for all chats in this project.</p>
                    </div>
                    ...
                    <div
                        ...
                        class="flex items-center gap-2 text-green-400 text-sm bg-green-400/10 border border-green-400/20 rounded-lg px-4 py-2"
                    >
